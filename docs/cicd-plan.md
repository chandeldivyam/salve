...
- [ ] **Inngest plan**: upgrade to Pro ($20/mo) and restore the original limits.
  - `deliver-message` concurrency: `{ workspaceID: 5, "ses": 5 }` → `{ workspaceID: 50, "ses": 200 }`
  - `route-inbound-message` concurrency: `{ channelID: 5 }` → `{ channelID: 20 }`
  - `process-provider-webhook` batchEvents: `maxSize: 5` → `maxSize: 50`
  These were the values the agent-platform RFC committed to.
